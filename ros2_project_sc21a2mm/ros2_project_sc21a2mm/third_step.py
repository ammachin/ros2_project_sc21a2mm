...
class colourIdentifier(Node):
    ...
    def callback(self, data):
        ...
        # findContours doesn't support 3 channel images
        grey_img = cv2.cvtColor(green_img, cv2.COLOR_BGR2GRAY)
        # Find the contours that appear within the certain colour mask using the cv2.findContours() method
        # For <mode> use cv2.RETR_LIST for <method> use cv2.CHAIN_APPROX_SIMPLE
        contours, _ = cv2.findContours(grey_img, mode=cv2.RETR_LIST, method=cv2.CHAIN_APPROX_SIMPLE)
        
        # Init green_found to false everytime
        self.green_found = False
        ...
        if len(contours) > 0:
            ...
            aValue = 10 #?
            if cv2.contourArea(c) > aValue: #<What do you think is a suitable area?>
                ...
                cv2.circle(image,(cx,cy),int(radius),colour,thickness)
